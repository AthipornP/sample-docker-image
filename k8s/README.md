...
Structure:
- namespace.yaml  -> creates namespace `customer_portal`
...
- All Deployments are in namespace `customer_portal`.
...
Apply locally with kubectl:
  ...
  kubectl apply -f svelte-portal/ -n customer_portal
  kubectl apply -f django-web/ -n customer_portal
  kubectl apply -f php-web/ -n customer_portal
